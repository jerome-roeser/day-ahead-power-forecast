...
from day_ahead_power_forecast.ml_ops.encoders import time_features_encoder, wind_features_encoder


def preprocess_PV_features(X: pd.DataFrame) -> np.ndarray:
    time_encoder = FunctionTransformer(
        time_features_encoder,
        kw_args={'time_column_name': 'local_time'})
...
def preprocess_forecast_features(X: pd.DataFrame) -> np.ndarray:
    ...
    time_encoder = FunctionTransformer(
        time_features_encoder,
        kw_args={'time_column_name': 'prediction_utc_time'})
